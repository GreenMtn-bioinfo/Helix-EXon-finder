...
#TODO: Ass a structural profile visualization script?
...
parser.add_argument('-t', '--threshold', type=float, default=0.75, metavar='<probability_threshold_float>',
                    help='Exon-level probability score threshold for reporting an exon prediction (default is 0.75). Please see the README and/or manuscript for guidance on threshold selection.')
...
import profile_generator_inference as pgi
import numpy as np
import pandas as pd
import time
import json
...
temp_dir = '../temp/'
norm_data_path = [f'{data_dir}normalization_params/z_norm_training_means.npy', 
                  f'{data_dir}normalization_params/z_norm_training_sdevs.npy', 
                  f'{data_dir}normalization_params/min_training_post_z.npy', 
                  f'{data_dir}normalization_params/max_training_post_z.npy']

# Import exon length distribution data for exon-level prediction re-ranking
length_dist = pd.read_csv(f'{data_dir}human_exon_length_distribution_Mokry_et_al_2010.csv')
...
def prepare_model(model_name: str, 
    ...
    weights_path = f'./{models_dir}/{model_name}_best.keras'
    args_path = f'./{models_dir}/{model_name}_training_call.json'
...
def split_n_predict(profile: np.ndarray, 
    ...
    Args:
        ...
        model: A Keras model object with weights fully loaded, must be one of the three trained during the work described in the manuscript.
...
def length_prob(x: int, 
    ...
    """
    Looks up the probability of a length of an exon based on an approximate human distribution estimated from Mokry et al. (2010), see manuscript.
    """
...
def make_exon_predictions(decisions: np.ndarray, 
    ...
    NOTE: This approach is a crude stand in for more complex model(s) that have also effectively learned the relevant gene structural/length distributions (see the manuscript)
    ...
    Args:
        ...
        max_exon_length: This function will not pass and exon prediction longer than max_exon_length (where the length distribution estimated from Mokry et al. ended), this is a major limitation (see the manuscript)!
...
if __name__ == '__main__':
    ...
    if pgi.log_file_name in os.listdir(profiles_dir):
        print(Fore.YELLOW + f"WARNING: One or more sequences were skipped during profile generation, see '{profiles_dir + pgi.log_file_name}' for details.")
    ...
    for i, profile_file in enumerate(profiles_list):
        profile = np.load(f'{profiles_dir}{profile_file}', mmap_mode='r')
        profile_lengths[profile_file] = profile.shape[1]
    ...
    for i, profile_file in enumerate(profiles_list):
        ...
        if i > 0:
            ...
            for remaining_file in remaining_profiles:
                rem_len = profile_lengths[remaining_file]
        ...
        full_msg = f'Processing profile {i+1} of {len(profiles_list)}: {profile_file}. Rough ETC: {msg}'
        full_msg = pad_message(full_msg, msg_length)
        if i == 0:
        ...
        seq_id = profile_file.replace(".npy", "")
        ...
        write_exons_to_gff(gff_path=f'{predictions_dir}{profile_file.replace(".npy", ".gff")}', 
        ...
        duration = end - start
        current_len = profile_lengths[profile_file]
    ...
    if delete_profiles_after:
        shutil.rmtree(profiles_dir)
        os.mkdir(profiles_dir)
        print(Fore.GREEN + f"Structural profiles in '{profiles_dir}' have been deleted as per user request.")
    else:
