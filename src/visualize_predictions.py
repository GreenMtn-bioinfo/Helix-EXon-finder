...
parser.add_argument('-p','--predictions_dir', type=str, default='../predictions/', metavar='<predictions_directory_path>',
                    help="Path to a directory containing GFFs with predictions generated by HEX-finder (defaults is '../predictions/').")
parser.add_argument('-t','--truth_features', type=str, default=None, metavar='<path_to_local_coords_GFF>',
                    help="Path to a single GFF file containing the local 1-based coordinates (wrt sequence start) of known reference features for all of the sequences of interest. Sequence IDs in the GFF must match those pulled by HEX-finder from the FASTA headers. See the GFFs provided in '../demo_sequences/' for valid formatting. While not necessary, the 9th column (i.e. attributes field) can contain annotation used to label individual reference features (see <feature_attribute_name>). If left unspecified, only the HEX-finder predictions will be plotted.")
parser.add_argument('-tl','--truth_labels_attribute', type=str, default=None, metavar='<feature_attribute_name>',
                    help="Name of the attribute in the attributes field/column of the truth GFF from which to pull individual feature labels (see <path_to_local_coords_GFF> above). Attributes field must use ';' for attribute separation and '=' for attribute assignment. See the GFFs provided in '../demo_sequences/' for valid examples. For RefSeq annotation, 'gene' will label each feature with its HUGO gene symbol. If left unspecified, each reference feature will simply be labeled as what is provided for <truth_source_name>.")
parser.add_argument('-ts','--truth_source_name', type=str, default=None, metavar='<reference_features_source>',
                    help="Name of the source of reference annotation HEX-finder is being compared to (for the axis labels/legend, e.g. 'BestRefSeq'). If unspecified, the script attempts to pull this from the 'source' field of the GFF file at truth_features (if provided and non-empty). If a homogenous source name is not found, the label defaults to 'Reference'.")
parser.add_argument('-o','--output_path', type=str, default='../predictions/visual_report.html', metavar='<output_HTML_path>',
                    help="Path to write the final HTML report to (the default is '../predictions/visual_report.html').")
...
truth_features_path = args.truth_features
...
favicon_path = '../dependencies/hex_finder_simple_icon.png'
...
if __name__ == '__main__':
    ...
    gff_files = sorted([f for f in os.listdir(predictions_dir) if f.endswith('.gff')])
    ...
    for i, prediction in enumerate(gff_files):
        ...
        seq_id, seq_source, seq_type, seq_length, predicted_exons = parse_gff(f'{predictions_dir}{prediction}')
        ...
        if truth_features_path:
            ...
            _, _, _, _, truth_features = parse_gff(f'{temp_dir}reference_features_temp.gff')
