...
parser.add_argument('-r','--reference_dir', type=str, default='../data/reference_genome/', metavar='<reference_genome_directory>',
                    help='Path to a directory including an indexed reference genome file (FNA and FAI) for GRCh38.p14, along with a RefSeq annotation file (GFF). The default is "../data/reference_genome/". If any of the required files are not found, the user will be prompted to give permission to retrieve and prepare them automatically.')
...
use_case.add_argument('-c','--coordinates', type=str, default=None, metavar='<coordinates_file>',
                      help='Path to a file including one genomic coordinate per line. Coordinate format should be CHROMOSOME:START-END(STRAND_SYMBOL), please see "../demo_sequences/Figure11_demo_regions" for a valid example.')
...
sampling_group.add_argument('-l', '--lengths', type=str, default=None, metavar='<lengths_distribution_file>',
                            help='Path to a file with one sequence length per line (integers). This will be used as the distribution of possible lengths for the random coordinate sampling. Please see or use "../demo_sequences/example_length_distribution" as a valid example.')
sampling_group.add_argument('-o', '--output', type=str, default=None, metavar='<output_base_name>',
                            help='The base name to use for all output files if random sampling is chosen (defaults to "<number_to_sample>_seqs"). All files will be saved in "../demo_sequences/".')
...
# A default base directory assumed for file writing purposes
operating_dir = '../demo_sequences/'

# Relative path to the file that contains the regions of GRCh38.p14 omitted from the models' training data
held_out_file_path = "../data/held_out_regions_GRCh38_p14.txt"

# Define paths of temporary files that are created as necessary intermediates and then deleted
temp_dir = f'{operating_dir}temp/'
temp_region_path = f'{temp_dir}temp_region.txt'
temp_gff_path = f'{temp_dir}temp_coords.gff'

# This is used to write output if random sampling was chosen
sampled_coords_path = f'{operating_dir}{output_name}'
...
def check_ref_dir(reference_dir: str) -> tuple:
    ...
    for file_name in reference_files:
        ...
        match suffix:
            case '.fna':
                reference_fasta_path = f'{reference_dir}{file_name}'
            case '.fai':
                reference_fasta_index = f'{reference_dir}{file_name}'
            case '.gff':
                annotation_gff_path = f'{reference_dir}{file_name}'
...
def BED_intersect(ref_features_path: str, # truth_gff
    ...
    if '.gff' not in ref_features_path or '.gff' not in range_path:
...
def fetch_truth_features(chrom: str,
    ...
    truth_temp_file_path = f'{temp_dir}{seq_id}_truth_features.gff'
...
if __name__ == '__main__':
    ...
    command = f"cat {annotation_gff_path} | grep -P '\tBestRefSeq\t' | grep -P 'tag=MANE\\sSelect' | grep -P '\texon\t' | grep '^NC_' > {temp_dir}mane_select_exons.gff"
    ...
    for strand_symbol in ['-', '+']:
        ...
        command = f"cat {temp_dir}mane_select_exons.gff | grep -P '\t\{strand_symbol}\t' > {temp_dir}mane_select_{strand}_exons.gff"
    ...
    truth_features_path = f'{chosen_file}_truth_features_local.gff'
    output_fasta = f'{chosen_file}.fasta'
    ...
    for i, region in enumerate(chosen_regions):
        ...
        # Fetch this sequence from the reference FASTA and append to output file
        strand_flag = '-i' if strand_symbol == "-" else ''
        ...
        fetch_truth_features(chrom = chromosome,
                             ...
                             truth_annotation_path = f"{temp_dir}mane_select_{'reverse' if strand_symbol == '-' else 'forward'}_exons.gff",
                             output_path = truth_features_path)
    
    if not quiet:
        print(Fore.GREEN + f"Sequences successfully retrieved for all valid coordinates and written to '{output_fasta}'.")
        print(Fore.GREEN + f"Overlapping exon annotation was retrieved, converted to local coordinates, and written to '{truth_features_path}'.")
